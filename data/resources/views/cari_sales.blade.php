...
<script type="text/javascript">
    ...
    var count=1;
     var ket='';
      ...
      if(srtwitel !=""){
        ...
        $.ajax({
          type:"post",
          url:"{{URL::to('sortir_witel_sales')}}",
          data:{"_token": "{{ csrf_token() }}",'srtwitel': srtwitel,'status': status},
          success: function(data){  
          $('#hasil_sortir').html(data);    

            console.log(srtwitel);
            $.each(data['hasil_sortir'], function(k, v) {
			if(v.ID_User !=null){
				ket = '<span class="label label-pill label-success">ACTIVE</span>';
			}else{
				ket = '<span class="label label-pill label-error">NON ACTIVE</span>'
			}
  					$('#hasil_sortir').append(                
                    "<tr>"                    
                    +"<td>"+count+"</td>"
                      ...
                      +"<td>"+v.datel+"</td>"
                       ...
                       +"<td>"+v.agency+"</td>"
                       +"<td>"+v.kode_agency+"</td>"
           +"<td><img src='data/storage/profil/"+v.foto_profil+"'width='80'></td>"
                       +"<td>"+v.role+"</td>"
                       +"<td>"+ket+"</td>"
                       +"<td><div class='btn-group btn-group-sm' style='float: none;'>"
	+"<a href='"+v.id+"' class='btn btn-success btn-sm' data-toggle='modal' data-target='.detail"+v.id+"' data-placement='bottom' title='Detail'><i class='fa fa-search-plus'></i></a>"
	+"<a href='#' class='btn btn-info btn-sm' data-toggle='modal' data-target='.ubah_password"+v.id+"' data-placement='bottom' title='Ubah Password'><i class='fa fa-unlock-alt'></i></a>"
	...
	+"<a href='hapus/"+v.id+"' class='btn btn-danger btn-sm' data-toggle='tooltip'"
   ...
   {{ csrf_field() }}
	...
	<select name="witel" id="srtwitel" onchange="sortir_witel()" class="form-control">
					...
					@endforeach
							...
							<p class="form-control-static">
	<select name="status" id="status" onchange="sortir_witel()" class="form-control">
	...
	<form role="form" action="{{URL::to('cari_sales')}}" method="post" enctype="multipart/form-data">
   {{ csrf_field() }}
				<div class="row">
					<div class="col-md-6">
			<div class="btn-group btn-group-sm" style="float: none;">
	...
	<select name="status"class="form-control">
					...
					<option value="2">Non Active</option>
							...
							<div style="margin-right: 3px"></div>						
				<p class="form-control-static">
					<button type="submit" name="save" class="btn btn-inline btn-warning">Cari</button>
				</p>
